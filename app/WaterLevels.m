...
%      
    properties (Hidden)
        ...
        %properties in struct used to call simple_tide but not used in ChannelForm
    ...
    properties (Transient)
        ...
        dMWchange           %change over a time step at mean water level (slr only)
...
%%
        function [HWL,MSL,LWL] = netChangeWL(obj,robj)
            %calculate the net change in water levels over the simulation 
            %period from t=0 to t=nstep.dt 
            rnpobj = robj.RunParam.RunProperties;
            y2s = robj.cns.y2s;
            dt = rnpobj.TimeStep;     %or robj.delta in seconds
            nstep = rnpobj.NumSteps;            
            mtime = [0,nstep*dt]*y2s;
            [HW,MS,LW] = newWaterLevels(obj,mtime,rnpobj.StartYear*y2s);
            HWL = HW(2)-HW(1);
            MSL = MS(2)-MS(1);
            LWL = LW(2)-LW(1);
        end
%%
        function dWl = delWL(obj,robj,t)
            %change in waterlevels over interval dt = t(2)-t(1), t in years 
            %with non-linear slr using the rate at time t can lead to 
            %errors if the time step is large. Actual change can be used to
            %obtain an equivalent linear rate over the interval dt.
            %NB: properties d**change are from previous time step to time t
            %this function allows user to specify the time interval
            rnpobj = robj.RunParam.RunProperties;
            y2s = robj.cns.y2s;          
            [HW,MS,LW] = newWaterLevels(obj,t*y2s,rnpobj.StartYear*y2s);
            dWl.HWL = HW(2)-HW(1);
            dWl.MSL = MS(2)-MS(1);
            dWl.LWL = LW(2)-LW(1);
        end
